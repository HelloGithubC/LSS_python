 ...
 */
template <typename T>
void smu_cosmo_convert(T s_f, T mu_f, T DA_f, T DA_t, T H_f, T H_t, T& s_t, T& mu_t) {
    T s_r = s_f * mu_f;
    T s_p = s_f * std::sqrt(T(1.0) - mu_f * mu_f);
    T alpha_p = DA_t / DA_f;
    T alpha_r = H_f / H_t;
    s_t = std::sqrt((alpha_p * s_p) * (alpha_p * s_p) + (alpha_r * s_r) * (alpha_r * s_r));
    mu_t = alpha_r * s_r / (s_t + T(1e-15));
}
 ...
 */
template <typename T>
void find_dense_in_sparse(
    const T s_points_sparse[2], const T mu_points_sparse[2],
    T smin_all, T smax_all, T mumin_all, T mumax_all,
    T ds_dense, T dmu_dense,
    int& i_s_min, int& i_s_max, int& i_mu_min, int& i_mu_max
) {
    T s_min = std::min(s_points_sparse[0], s_points_sparse[1]);
    T mu_min = std::min(mu_points_sparse[0], mu_points_sparse[1]);
    T s_max = std::max(s_points_sparse[0], s_points_sparse[1]);
    T mu_max = std::max(mu_points_sparse[0], mu_points_sparse[1]);
...
T get_rate(
    ...
    T s_sparse_low, T s_sparse_high, T mu_sparse_low, T mu_sparse_high,
    const T smu_dense_points[4][2]  // [point_index][s_or_mu], order: (0,0), (1,0), (0,1), (1,1)
) {
    if (outpoint == 0) return T(1.0);
    if (outpoint == 4) return T(0.0);
    
    T dense_s = smu_dense_points[1][0] - smu_dense_points[0][0];
    T dense_mu = smu_dense_points[2][1] - smu_dense_points[0][1];
    T dense_area = dense_s * dense_mu;
    
    if (outpoint == 2) {
        if (scope == s_scope * 2) {
            T in_area = (T(2) * s_sparse_high - smu_dense_points[0][0] - smu_dense_points[2][0]) 
                       * (smu_dense_points[2][1] - smu_dense_points[0][1]) / T(2.0);
            return in_area / dense_area;
        } else if (scope == -s_scope * 2) {
            T in_area = (smu_dense_points[1][0] + smu_dense_points[3][0] - T(2) * s_sparse_low) 
                       * (smu_dense_points[3][1] - smu_dense_points[1][1]) / T(2.0);
            return in_area / dense_area;
    ...
    if (outpoint == 3) {
        ...
        if (scope == -(s_scope + mu_scope) * 2 || scope == -(s_scope + mu_scope) * 2 + s_scope || scope == -(s_scope + mu_scope) * 2 - s_scope) {
            T high_temp = smu_dense_points[3][1] - mu_sparse_low;
            T top_line = smu_dense_points[3][0] - s_sparse_low;
            T middle_line = top_line + (smu_dense_points[1][0] - smu_dense_points[3][0]) * high_temp / dense_mu;
            T in_area = (top_line + middle_line) * high_temp / T(2.0);
            return in_area / dense_area;
        }
        if (scope == T(2) * (mu_scope - s_scope) || scope == T(2) * (mu_scope - s_scope) - s_scope || scope == T(2) * (mu_scope - s_scope) + s_scope) {
            T high_temp = mu_sparse_high - smu_dense_points[1][1];
            T tail_line = smu_dense_points[1][0] - s_sparse_low;
            T middle_line = tail_line + (smu_dense_points[3][0] - smu_dense_points[1][0]) * high_temp / dense_mu;
            T in_area = (tail_line + middle_line) * high_temp / T(2.0);
            return in_area / dense_area;
        }
        if (scope == (s_scope + mu_scope) * 2 || scope == (s_scope + mu_scope) * 2 - s_scope || scope == (s_scope + mu_scope) * 2 + s_scope) {
            T high_temp = mu_sparse_high - smu_dense_points[0][1];
            T tail_line = s_sparse_high - smu_dense_points[0][0];
            T middle_line = tail_line + (smu_dense_points[2][0] - smu_dense_points[0][0]) * high_temp / dense_mu;
            T in_area = (tail_line + middle_line) * high_temp / T(2.0);
            return in_area / dense_area;
        }
        if (scope == T(2) * (s_scope - mu_scope) || scope == T(2) * (s_scope - mu_scope) - s_scope || scope == T(2) * (s_scope - mu_scope) + s_scope) {
            T high_temp = smu_dense_points[2][1] - mu_sparse_low;
            T top_line = s_sparse_high - smu_dense_points[2][0];
            T middle_line = top_line + (smu_dense_points[0][0] - smu_dense_points[2][0]) * high_temp / dense_mu;
            T in_area = (top_line + middle_line) * high_temp / T(2.0);
            return in_area / dense_area;
        }
        return T(0.0);
 ...
 */
template <typename T>
std::vector<T> mapping_smudata_dense_cpp(
    const std::vector<T>& smutabstd,  // shape: (sbin_dense, mubin_dense, element_size)
    size_t sbin_dense, size_t mubin_dense,
    size_t sbin_sparse, size_t mubin_sparse,
    T Hz_f, T Hz_m, T DA_f, T DA_m,
    T smin_all, T smax_all, T mumin_all, T mumax_all,
    T smin_mapping, T smax_mapping,
    int nthreads = 1
) {
    // Calculate bin sizes
    T delta_s = smax_all - smin_all;
    T delta_mu = mumax_all - mumin_all;
    
    T ds_dense = delta_s / sbin_dense;
    T ds_sparse = delta_s / sbin_sparse;
    T dmu_dense = delta_mu / mubin_dense;
    T dmu_sparse = delta_mu / mubin_sparse;
    ...
    std::vector<T> smutab2(sbin_sparse * mubin_sparse * element_size, T(0.0));
    ...
    for (size_t i_s_sparse = 0; i_s_sparse < sbin_sparse; ++i_s_sparse) {
        for (size_t i_mu_sparse = 0; i_mu_sparse < mubin_sparse; ++i_mu_sparse) {
            ...
            T s_sparse_2 = smin_all + i_s_sparse * ds_sparse;
            T mu_sparse_2 = mumin_all + i_mu_sparse * dmu_sparse;
            ...
            T s_points_sparse_2[2], mu_points_sparse_2[2];  // Only need 2 points: min and max s
            
            for (int s_add = 0; s_add <= 1; ++s_add) {
                for (int mu_add = 0; mu_add <= 1; ++mu_add) {
                    T s_temp = s_sparse_2 + s_add * ds_sparse;
                    T mu_temp = mu_sparse_2 + mu_add * dmu_sparse;
            ...
            if (need_convert) {
                ...
                T s_points_dense[2], mu_points_dense[2];
            ...
            std::vector<T> smu_points_dense_2(s_cubes_size * mu_cubes_size * 2, T(0.0));
            ...
            for (int is_dense = i_s_dense_min; is_dense < i_s_dense_max + i_s_add_temp; ++is_dense) {
                for (int imu_dense = i_mu_dense_min; imu_dense < i_mu_dense_max + i_mu_add_temp; ++imu_dense) {
                    T s_dense_1 = smin_all + is_dense * ds_dense;
                    T mu_dense_1 = mumin_all + imu_dense * dmu_dense;
                    
                    T s_dense_2, mu_dense_2;
                    if (need_convert) {
                    ...
                    // Clamp to bounds
                    if (s_dense_2 >= smax_all) s_dense_2 = smax_all - T(1e-8);
                    if (s_dense_2 < smin_all) s_dense_2 = smin_all + T(1e-8);
                    if (mu_dense_2 >= mumax_all) mu_dense_2 = mumax_all - T(1e-8);
                    if (mu_dense_2 < mumin_all) mu_dense_2 = mumin_all + T(1e-8);
            ...
            T s_sparse = ds_sparse * i_s_sparse;
            T mu_sparse = dmu_sparse * i_mu_sparse;
            T s_sparse_low = s_sparse;
            T s_sparse_high = s_sparse + ds_sparse;
            T mu_sparse_low = mu_sparse;
            T mu_sparse_high = mu_sparse + dmu_sparse;
            
            std::vector<T> rates_dense(s_cubes_dense_size * mu_cubes_dense_size, T(0.0));
            ...
            if (need_convert) {
                for (int i_s_cube = 0; i_s_cube < s_cubes_dense_size; ++i_s_cube) {
                    for (int i_mu_cube = 0; i_mu_cube < mu_cubes_dense_size; ++i_mu_cube) {
                        int scope_temp = 0;
                        int outpoint_temp = 0;
                        T smu_dense_points[4][2];  // Order: (0,0), (1,0), (0,1), (1,1)
                ...
                for (int i_s_cube = 0; i_s_cube < s_cubes_dense_size; ++i_s_cube) {
                    for (int i_mu_cube = 0; i_mu_cube < mu_cubes_dense_size; ++i_mu_cube) {
                        ...
                        T s_point = smu_points_dense_2[idx * 2 + 0] + T(0.5) * ds_dense;
                        // Note: Use dmu_sparse here, not dmu_dense (matches Python line 1002)
                        T mu_point = smu_points_dense_2[idx * 2 + 1] + T(0.5) * dmu_sparse;
                        ...
                        if (i_s_sparse_2 == static_cast<int>(i_s_sparse) && i_mu_sparse_2 == static_cast<int>(i_mu_sparse)) {
                            rates_dense[i_s_cube * mu_cubes_dense_size + i_mu_cube] = T(1.0);
                        } else {
                            rates_dense[i_s_cube * mu_cubes_dense_size + i_mu_cube] = T(0.0);
                        }
                    }
                }
            }
            ...
            for (int i_s_cube = 0; i_s_cube < s_cubes_dense_size; ++i_s_cube) {
                for (int i_mu_cube = 0; i_mu_cube < mu_cubes_dense_size; ++i_mu_cube) {
                    T rate = rates_dense[i_s_cube * mu_cubes_dense_size + i_mu_cube];
                    if (rate > T(1e-10)) {
